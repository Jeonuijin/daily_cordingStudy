...
-- DDL문 사용 테이블 생성
CREATE TABLE MEMBER (
    ID VARCHAR2(20) PRIMARY KEY, --NOT NULL + UNIQUE   --컬럼 (,)구분자로 나열하기
...
-- DCL : ROLLBACK, COMMIT
ROLLBACK; --임시처리된 작업 취소처리
COMMIT; --임시처리된 작업을 최종적용한다.

-- 키중복 데이터 입력시 : ORA-00001: unique constraint (MYSTUDY.SYS.C007002) violated
INSERT INTO MEMBER (ID, NAME, PASSWORD)
...
VALUES ('HONG3', '홍길동3');
----------------------------------
SELECT * FROM MEMBER; -- 테이블에 있는 모든 데이터 + 모든 컬럼(*사용)
SELECT ID, NAME, PASSWORD, PHONE FROM MEMBER; --보고싶은 항목만 선택해서 볼 수 있다.
----------------------
--입력 : 컬럼명 명시적으로 쓰지 않고 INSERT문 작성
--테이블 모든 컬럼 사용 + 컬럼 순서대로 데이터 입력
INSERT INTO MEMBER -- 괄호 생략(컬럼명을 명시적으로 쓰지 않으면 정의되어 있는 컬럼대로 입력해야함)
...
VALUES ('HONG5', '홍길동5', '1234', '010-2222-222', '서울');
COMMIT;
...
FROM MEMBER WHERE NAME = '홍길동8'; --보기좋게 정렬하기
---- 수정
UPDATE MEMBER
