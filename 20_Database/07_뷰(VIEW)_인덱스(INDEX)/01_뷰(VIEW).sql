...
-- 뷰(VIEW) 만들기 //자주 쓰는 결과값을 DB에 저장해서 사용할 수 있다.
CREATE VIEW VW_BOOK
AS
SELECT * FROM BOOK WHERE BOOKNAME LIKE '%축구%'
;
CREATE OR REPLACE VIEW VW_BOOK  --OR REPLACE 수정가능
AS
SELECT * FROM BOOK WHERE BOOKNAME LIKE '%축구%'
WITH READ ONLY   --읽기전용 //가져다 쓸 때
;
--뷰 사용해서 데이터 검색
SELECT * FROM VW_BOOK;
...
----------------------------
CREATE OR REPLACE VIEW VW_ORDERS             --가상테이블 만들기
AS
SELECT O.*,
...
FROM VW_ORD_ALL
WHERE PUBLISHER = '이상미디어' AND NAME IN ('추신수','장미란')
