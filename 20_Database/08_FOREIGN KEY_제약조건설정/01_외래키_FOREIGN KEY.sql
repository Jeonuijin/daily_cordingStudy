...
CREATE TABLE EMP01 (
    ...
    JOB VARCHAR2(10),
    DEPTNO VARCHAR2(10) REFERENCES DEPT (ID)  --외래키 설정(컬럼레벨에서)
);
SELECT * FROM EMP01;
INSERT INTO EMP01 (EMPNO, ENAME, JOB, DEPTNO)
VALUES (1111, '홍길동', '직무1','10');

--ORA-02291: integrity constraint (MADANG.SYS_C007020) violated - parent key not found
INSERT INTO EMP01 (EMPNO, ENAME, JOB, DEPTNO)
VALUES (2222, '홍길동2', '직무2','40'); -- 오류 / 참조하는 부모키의 값에 40이 없기 때문에
...
/* 테이블에 제약조건 추가, 삭제
-- 제약조건추가
ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건명 적용할 제약조건;
-- 제약조건삭제
ALTER TABLE 테이블명 DROP CONSRAINT 제약조건명;
*************************/
-- EMP01 테이블의 PRIMARY KEY 삭제 :
ALTER TABLE EMP01 DROP CONSTRAINT SYS_C007019;

-- EMP01 테이블의 제약조건 추가
ALTER TABLE EMP01 ADD CONSTRAINT EMP01_EMPNO_PK PRIMARY KEY(EMPNO);
-------------------------------------------------
-- EMP02, EMP03 테이블 외래키(FOREIGN KEY) 이름 변경처리
-- EMP02 : EMP02_EMPNO_FK, EMP03 : FK_EMP03_DEPTNO

-- EMP02 
ALTER TABLE EMP02 DROP CONSTRAINT SYS_C007023; --삭제
ALTER TABLE EMP02 ADD CONSTRAINT EMP02_EMPNO_FK --추가
...
--제약조건 추가 또는 활성화(적용) 시
----제약조건 활성화(또는 추가시) 제약조건 위반 데이터가 있으면 활성화(추가) 안됨
...
CREATE TABLE HSCHOOL (
    ...
    BUN NUMBER(2), --번
    NAME VARCHAR2(30),
    CONSTRAINT HSCHOOL_HAK_BAN_BUN_PK PRIMARY KEY (HAK, BAN, BUN) --3개의 값을 모았을때 유니크한 값이 될 수 있다.
);
SELECT * FROM HSCHOOL;
INSERT INTO HSCHOOL VALUES (1,1,1,'홍길동');
INSERT INTO HSCHOOL VALUES (1,1,2,'강감찬');
