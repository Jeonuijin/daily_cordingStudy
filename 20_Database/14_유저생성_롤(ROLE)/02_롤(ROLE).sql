...
--(관리자계정-SYSTEM)
SELECT *
FROM DBA_SYS_PRIVS      --DBA권한이 있는 유저만 가능(관리자계정)
WHERE GRANTEE IN ('CONNECT', 'RESOURCE')
ORDER BY GRANTEE, PRIVILEGE
;

----------------------------------------------
--(SYSTEM) 롤(ROLE) 생성 : PROGRAMMER(개발자) 라는 역할(롤)
CREATE ROLE PROGRAMMER;

--(SYSTEM) PROGRAMMER 에게 권한 부여 테이블, 뷰(VIEW) 생성권한 부여
GRANT CREATE ANY TABLE, CREATE ANY VIEW TO PROGRAMMER;  --ANY TABLE : 다른유저에도 뷰를 생성할 수 있다.

--(SYSTEM) MDGUEST 유저에게 PROGRAMMER(롤) 권한을 부여
...
CREATE TABLE MADANG.BBB (BBB NUMBER); --테이블생성 성공
CREATE TABLE MYSTUDY.BBB (BBB NUMBER); --테이블생성 성공
