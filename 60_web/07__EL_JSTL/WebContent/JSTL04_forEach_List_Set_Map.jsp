...
<%
	...
	set.add("강감찬");
	set.add("을지문덕");
	set.add("을지문덕");		//set은 같은데이터는 1번만 출력된다.(중복제거)
...
<%
	...
	map.put("k3","abc");
	map.put("k4","abcd");
	map.put("k4","abcd~~");		//Map은 동일 데이터가 있으면 나중에 저장된 데이터로 변경된다.
	System.out.println("map : " + map);
	pageContext.setAttribute("hmap", map);
%>
	<p>--- Map 특정 데이터 key값으로 조회---</p>
	\${hmap.k1} : ${hmap.k1 }<br>
	\${hmap.k2} : ${hmap.k2 }<br>
	\${hmap["k3"]} : ${hmap["k3"] }<br>
	\${hmap['k4']} : ${hmap['k4'] }<br>
...
<%
	...
	listBean.add(new PersonVO("김유신","44"));
	listBean.add(new PersonVO("강감찬","33"));
...
%>
	...
	<c:forEach var="value" items="${persons }">
		${value }<br>
		이름 : ${value.name }<br>
		나이 : ${value.age }<br>
		--------------------<br>
