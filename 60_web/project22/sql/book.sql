...
--BOOK테이블 변경
CREATE SEQUENCE BOOKID_SEQ NOCACHE;		--책아이디시퀀스 추가
CREATE SEQUENCE REVIEW_ID_SEQ NOCACHE;	--리뷰아이디 시퀀스 추가
ALTER TABLE BOOK
ADD REVIEW_ID NUMBER(3) REFERENCES BOOK_REVIEW (REVIEW_ID);	--REVIEW_ID 외래키 설정

ALTER TABLE BOOK_REVIEW	--BOOK_REVIEW 테이블에 REGDATE칼럼 추가
ADD REGDATE DATE;

ALTER TABLE BOOK	--BOOK테이블에 평점평균 삭제
DROP COLUMN GRADE_AVG;

ALTER TABLE BOOK  	--BOOK 수량 NUMBER타입변경
MODIFY (BOOK_CNT NUMBER(5, 0) );

ALTER TABLE BOOK		--책 상태값 수정
