...
<script>
	function sendData() {
		alert("sendDate() 시작");
		let firstForm = document.forms[0];  //form중에 첫번째 form객체 선택
		alert("firstForm.elements.length : " + firstForm.elements.length); //선택한 form객체의 요소 갯수
		for (let i = 0; i < firstForm.elements.length; i++){
			if (firstForm.elements[i].value.trim() == "") {	//값을 trim처리한 것이 빈문자열이냐?
				console.log(firstForm.elements[i]);
				//if (i == 3) continue; //첨부파일은 제외
				if (firstForm.elements[i].getAttribute("type") == "file") continue; //첨부파일은 제외
