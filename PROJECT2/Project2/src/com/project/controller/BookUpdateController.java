...
public class BookUpdateController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String gradeAvgParam = mr.getParameter("gradeAvg");
		String bookCntParam = mr.getParameter("bookCnt");
		String orderCntParam = mr.getParameter("orderCnt");

		try {
		    if (gradeAvgParam != null) {
		        vo.setGradeAvg(Double.parseDouble(gradeAvgParam));
		    } else {
		        vo.setGradeAvg(0.0); // 기본값 설정 또는 오류 처리
		    }

		    if (bookCntParam != null) {
		        vo.setBookCnt(Integer.parseInt(bookCntParam));
		    } else {
		        vo.setBookCnt(0); // 기본값 설정 또는 오류 처리
		    }

		    if (orderCntParam != null) {
		        vo.setOrderCnt(Integer.parseInt(orderCntParam));
		    } else {
		        vo.setOrderCnt(0); // 기본값 설정 또는 오류 처리
		    }
		} catch (NumberFormatException e) {
		    // 파싱 오류 처리
		    e.printStackTrace(); // 또는 오류 로그 작성
		}
		...
		if (adminBookDAO.update(vo) == 1) {
		    ...
		    resp.getWriter().write("저장에 실패했습니다");
		}
